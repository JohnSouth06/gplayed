...
switch ($action) {
    ...
    case 'api_get_public_collection':
        ...
        $gameController = new GameController($db);
        // On peut réutiliser la logique existante ou filtrer ici
        $stmtGames = $db->prepare("SELECT * FROM games WHERE user_id = ? AND status NOT IN ('wishlist', 'loaned') ORDER BY id DESC");
