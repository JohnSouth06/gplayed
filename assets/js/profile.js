...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnSyncPsn) {
        btnSyncPsn.addEventListener('click', async () => {
            ...
            try {
                // Récupération du token de l'utilisateur (à adapter selon votre système de stockage)
                const userToken = localStorage.getItem('token');

                // 2. Appel à l'API PHP (qui fera le relai vers le VPS)
                const response = await fetch('/api/index.php?action=api_psn_sync', {
                    method: 'GET', // ou POST selon vos standards
                    headers: {
                ...
                if (data.success) {
                    ...
                    // Optionnel : Recharger la page ou rafraîchir la liste des trophées dans le DOM
