            ...
            <div class="card-body p-5">
                <div class="text-center mb-4">
                    <div class="d-block pb-2 mx-auto w-75">
                        <svg id="logo-login" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 200 42">
                ...
                <ul class="nav nav-pills nav-fill mb-4 bg-transparent rounded-pill p-1">
                    <li class="nav-item px-1">
                        <button class="nav-link nav-login rounded-pill m-0 <?= $activeTab === 'login' ? 'active' : '' ?>" data-bs-toggle="tab" data-bs-target="#login-tab"><?= __('auth_signin') ?></button>
                    </li>
                    <li class="nav-item px-1">
                        <button class="nav-link nav-login rounded-pill m-0 <?= $activeTab === 'register' ? 'active' : '' ?>" data-bs-toggle="tab" data-bs-target="#register-tab"><?= __('auth_signup') ?></button>
                    </li>
                </ul>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade <?= $activeTab === 'login' ? 'show active' : '' ?>" id="login-tab">
                        <form action="/?action=login" method="POST">
                            ...
                            <div class="d-grid gap-2 col-12 mx-auto">
                                <button type="submit" class="btn btn-primary min-w-50 rounded-pill mt-4 px-3 py-2 fw-bold mx-auto"><i class="material-icons align-middle fs-5 me-2">&#xea77;</i><?= __('auth_signin') ?></button>
                            </div>
                        </form>
                    ...
                    <div class="tab-pane fade <?= $activeTab === 'register' ? 'show active' : '' ?>" id="register-tab">
                        <form action="/?action=register" method="POST">
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" name="username" class="form-control rounded-3" placeholder="Pseudo" required>
                                <label><?= __('auth_choose_pseudo') ?></label>
                            </div>
                            <div class="form-floating mb-3">
                            ...
                            <div class="d-grid gap-2 col-12 mx-auto">
                                <button type="submit" class="btn btn-primary min-w-50 rounded-pill mt-4 px-3 py-2 fw-bold mx-auto"><i class="material-icons align-middle fs-5 me-2">&#xe174;</i><?= __('auth_create_account') ?></button>
                            </div>
                        </form>
                        <?php
                        if (isset($_GET['error'])) {
                ...
                <div class="d-grid gap-2 col-12 mx-auto mb-3">
                    <a href="/?action=login_google" class="btn btn-light border min-w-50 rounded-pill py-2 fw-bold mx-auto d-flex align-items-center justify-content-center">
                ...
                <div class="d-grid gap-2 col-12 mx-auto mb-4">                    
                    <a href="/?action=login_discord" class="btn btn-light border min-w-50 rounded-pill py-2 fw-bold mx-auto d-flex align-items-center justify-content-center" style="background-color: #5865F2; color: white; border: none;">
...
<div class="modal fade" id="forgotPasswordModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <form action="/?action=forgot_password" method="POST">
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
                <div class="modal-header border-bottom-0">
                    <h5 class="modal-title fw-bold"><?= __('reset_title') ?></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p class="text-secondary small"><?= __('reset_message') ?></p>
                    <div class="form-floating mb-3">
                        <input type="email" name="email" class="form-control rounded-3" placeholder="Email" required>
                        <label><?= __('reset_email') ?></label>
                    </div>
                </div>
                <div class="modal-footer border-top-0">
                    <button type="submit" class="btn btn-primary min-w-50 rounded-pill mt-4 px-3 py-2 fw-bold mx-auto"><i class="material-icons align-middle fs-5 me-2">&#xf053;</i><?= __('reset_send_link') ?></button>
                </div>
            </form>
        </div>
    </div>
