...
class AuthController
{
    ...
    public function forgotPassword()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->userModel->emailExists($email)) {
                ...
                $link = $protocol . $domainName . $scriptDir . "//reset_password&token=" . $token;
    ...
    public function showResetForm()
    {
        ...
        // 1. Déterminer la langue (Session, ou défaut 'fr' si pas connecté/expiré)
        $langCode = $_SESSION['lang'] ?? 'fr';
        // Sécurité simple pour s'assurer qu'on charge un fichier existant
        if (!in_array($langCode, ['fr', 'en'])) {
            $langCode = 'fr';
        }

        // 2. Charger le fichier de langue approprié
        $t = require dirname(__DIR__) . '/lang/' . $langCode . '.php';
        
        // 3. Affichage du formulaire avec les traductions
        echo '<!DOCTYPE html>
                        ...
                        <div class="card-body p-5">
                            ...
                            <form action="/do_reset" method="POST">
                                ...
                                <div class="form-floating mb-4">
                                    ...
                                    <div class="form-text small mt-2"><i class="material-icons-outlined align-middle fs-6 me-1">info</i>' . $t['reset_help_text'] . '</div>
                                </div>
    ...
    public function doReset()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Chargement langue pour message erreur éventuel
            $langCode = $_SESSION['lang'] ?? 'fr';
