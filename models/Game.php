...
class Game
{
    ...
    public function save($data, $file, $userId)
    {
        ...
        if (!empty($data['game_id'])) {
            ...
            (user_id, game_id, platform, format, status, metacritic_score, user_rating, comment, description, dominant_color, estimated_price) 
            VALUES (:uid, :igdb_id, :platform, :format, :status, :meta, :rating, :comment, :desc, :color, :price)";
            $stmt = $this->conn->prepare($query);
