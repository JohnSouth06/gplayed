...
// 1. GESTION DES DEEP LINKS (Redirection vers l'app mobile)
if ($action === 'share' && !empty($_GET['username'])) {
    $appLink = "gplayed://profile/" . urlencode($_GET['username']);
    header("Location: /api/index.php?action=app_bounce&target=" . urlencode($appLink));
    exit;
}

if ($action === 'reset_password' && !empty($_GET['token'])) {
    $appLink = "gplayed://reset-password?token=" . urlencode($_GET['token']);
    header("Location: /api/index.php?action=app_bounce&target=" . urlencode($appLink));
    exit;
}

// 2. REDIRECTION PAR DÉFAUT
header("Location: https://www.g-played.com/api/index.php?action=app_bounce");
exit;
